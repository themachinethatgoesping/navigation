...
#include "nmea_vtg.hpp"

namespace themachinethatgoesping {
namespace navigation {
namespace nmea_0183 {

NMEA_VTG::NMEA_VTG(NMEA_Base base, bool check)
...
std::string NMEA_VTG::get_mode_explained() const
{
    switch (get_mode())
    {
        case 'A':
...
NMEA_VTG NMEA_VTG::from_stream(std::istream& is)
{
    return NMEA_VTG(NMEA_Base::from_stream(is), true);
}

tools::classhelper::ObjectPrinter NMEA_VTG::__printer__(unsigned int float_precision, bool superscript_exponents) const
{
