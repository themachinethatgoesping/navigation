...
class NMEA_GST : public NMEA_Base
{
   ...
   */
    ...
    : NMEA_Base(std::move(base))
    {
        if (check) {
            if(get_sentence_type() != "GST")
                throw std::runtime_error(fmt::format("NMEA_GST: wrong sentence type [{}]", get_sentence_type()));
        }
        parse_fields();
    }
    ...
    double get_psuedorange_rms() const
    {
        return get_field_as_double(1);
    }
    double get_semimajor_error() const
    {
        return get_field_as_double(2);
    }
    double get_semiminor_error() const
    {
        return get_field_as_double(3);
    }
    double get_semimajor_error_orientation() const
    {
        return get_field_as_double(4);
    }
    double get_latitude_error_deviation() const
    {
        return get_field_as_double(5);
    }
    double get_longitude_error_deviation() const
    {
        return get_field_as_double(6);
    }
    double get_altitude_error_deviation() const
    {
    ...
    tools::classhelpers::ObjectPrinter __printer__(unsigned int float_precision) const
    {
        ...
        printer.register_value("semimajor_error_orientation", get_semimajor_error_orientation(),"deg");
