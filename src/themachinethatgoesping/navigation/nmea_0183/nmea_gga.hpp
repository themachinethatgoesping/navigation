...
class NMEA_GGA : public NMEA_Base
{
   ...
   */
    ...
    : NMEA_Base(std::move(base))
    {
        if (check) {
            if(get_sentence_type() != "GGA")
                throw std::runtime_error(fmt::format("NMEA_GGA: wrong sentence type [{}]", get_sentence_type()));
        }
        parse_fields();
    }
    ...
    int get_number_of_satellites() const
    {
    ...
    double get_geoidal_separation() const
    {
        return get_field_as_double(10);
    }
    double get_age_of_differential_gps_data() const
    {
        return get_field_as_double(12);
    }
    int get_differential_reference_station_id() const
    {
    ...
    tools::classhelpers::ObjectPrinter __printer__(unsigned int float_precision) const
    {
        ...
        printer.register_value("horizontal_dilution_of_precision", get_horizontal_dilution_of_precision());
        printer.register_value("altitude", get_altitude(),"m");
        printer.register_value("geoidal_separation", get_geoidal_separation(),"m");
        printer.register_value("age_of_differential_gps_data", get_age_of_differential_gps_data(),"s");
        printer.register_value("differential_reference_station_id", get_differential_reference_station_id());
