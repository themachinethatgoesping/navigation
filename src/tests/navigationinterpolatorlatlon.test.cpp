...
TEST_CASE("NavigationInterpolatorLatLon should support common functions", TESTTAG)
{
    ...
    sensor_configuration.set_depth_source("", 0, 0, 1);
    sensor_configuration.set_attitude_source("", 1, -2, 3);
    sensor_configuration.set_position_source("", 10, -10, 5);
    ...
    datastructures::GeolocationLatLon expected_position(
        -10.7631, -1.16473, -6.6602486, 11.0909091, 0.7822953, -6.11409181148918992);
    ...
    INFO(std::to_string(target_position_mbes.latitude));
    INFO(std::to_string(target_position_mbes.longitude));

    REQUIRE(target_position_mbes == expected_position);
    REQUIRE(target_position_sbes == expected_position);
}

TEST_CASE("NavigationInterpolatorLatLon merging operations", TESTTAG)
{
    ...
    SECTION("Merge interpolators")
    {
        ...
        auto navint3 = navint;
        navint3.merge(NavigationInterpolatorLatLon(sensor_configuration));
        REQUIRE(navint == navint3);
        ...
        REQUIRE(navint3.interpolator_longitude().get_data_X() ==
                vector<double>({ 0, 1, 1.5, 1.6, 2, 2.9, 3 }));
        REQUIRE(navint3.interpolator_longitude().get_data_Y() ==
                vector<double>({ 1, -1, -1, -2, -2, 3, 3 }));
        REQUIRE(navint3.interpolator_latitude().get_data_X() ==
                vector<double>({ 0, 1, 1.5, 1.6, 2, 2.9, 3 }));
        REQUIRE(navint3.interpolator_latitude().get_data_Y() ==
...
TEST_CASE("NavigationInterpolatorLatLon get_sampled_timestamps", TESTTAG)
{
    // setup sensor configuration
    SensorConfiguration sensor_configuration;
    sensor_configuration.set_depth_source("gps", 0, 0, 1);
    ...
    SECTION("Default sensor names (latitude, longitude)")
    {
